...
type Field = {id: string; label: string; type: 'text' | 'email' | 'textarea' | 'select'; options?: string[]; wide?: boolean; required?: boolean};

const FIELDS: Field[] = [
  {id: 'name', label: 'Full name', type: 'text', required: true},
  {id: 'email', label: 'Email', type: 'email', required: true},
  {id: 'company', label: 'Company or organization', type: 'text', required: true},
  {id: 'role', label: 'Your role', type: 'select', options: enquiryRoles},
  {id: 'application', label: 'Application description', type: 'textarea', wide: true, required: true},
  ...
  {id: 'constraints', label: 'Key requirements and constraints', type: 'textarea', wide: true},
];

export default function Page() {
  const {go} = useNav();
  const [v, setV] = useState<Record<string, string>>({});
  const set = (id: string, value: string) => setV(p => ({...p, [id]: value}));
  ...
  return (
    <Shell route="contact">
      <section className="page-wrap">
        ...
        >
          <div className="dr-enq">
            {FIELDS.map(f => (
              <div className={'dr-enq-field' + (f.wide ? ' dr-enq-wide' : '')} key={f.id}>
                <label htmlFor={'enq-' + f.id}>{f.label}{f.required ? ' *' : ''}</label>
                {f.type === 'textarea' ? (
                  <textarea id={'enq-' + f.id} value={v[f.id] || ''} onChange={e => set(f.id, e.target.value)}/>
                ) : f.type === 'select' ? (
                  <select id={'enq-' + f.id} value={v[f.id] || ''} onChange={e => set(f.id, e.target.value)}>
                    <option value="">Select {f.label.toLowerCase()}</option>
                    {f.options!.map(o => <option key={o} value={o}>{o}</option>)}
                  </select>
                ) : (
                  <input id={'enq-' + f.id} type={f.type} value={v[f.id] || ''} onChange={e => set(f.id, e.target.value)}/>
                )}
              </div>
            ))}
            <div className="dr-enq-actions">
              <a className="primary" href={mailto}><Mail size={17} aria-hidden="true"/> Compose enquiry email</a>
              <a className="text-link" href={'mailto:' + CONTACT_EMAIL}>Or just write to {CONTACT_EMAIL} <ArrowUpRight size={16}/></a>
            </div>
            <p className="dr-enq-note">
              No analytics, no form processor and no cookies are involved. If your browser has no mail client
              configured, copy the address above instead.
            </p>
          </div>
        </Sec>
        ...
        >
          <div className="dr-hub-grid">
            ...
            <article className="dr-hub-card">
              ...
              <div className="dr-hub-footer">
                <button className="text-link" onClick={() => go('resources')}>Watch on Resources <ArrowUpRight size={16}/></button>
              </div>
            </article>
            <article className="dr-hub-card">
              ...
              <div className="dr-hub-footer">
                <button className="text-link" onClick={() => go('ask')}>Open the console <ArrowUpRight size={16}/></button>
              </div>
            </article>
          </div>
        </Sec>
        ...
        >
          <div className="dr-group">
            ...
            <div className="dr-links">
              ...
              <a className="text-link" href="https://github.com/shekerkamma/deepgrid-dr-silicon-v2" rel="noreferrer noopener" target="_blank">Source repository <ArrowUpRight size={16}/></a>
            </div>
          </div>
        ...
        <div className="dr-links dr-sec-gap">
          <button className="text-link" onClick={() => go('company')}>About DeepGrid Semi <ArrowUpRight size={16}/></button>
          <button className="text-link" onClick={() => go('evidence')}>How every figure is evidenced <ArrowUpRight size={16}/></button>
          <button className="text-link" onClick={() => go('procurement')}>Procurement information <ArrowUpRight size={16}/></button>
        </div>
      </section>
    </Shell>
  );
