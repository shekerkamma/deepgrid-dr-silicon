...
description: >
  ...
  identical and sacred — DG32 dark palette (ink ground, copper the one accent, teal only for
  the safe state), Georgia 400 display + Helvetica/Arial body, the 1px hairline as the only structural device, compass-drafted geometric rings,
...
colors:
  bg-primary: "#101212"      # DG32 ink: the page ground
  bg-secondary: "#191d1b"    # DG32 surface: panels, blocks
  text-primary: "#eeeae2"    # DG32 paper
  text-secondary: "#a7b09f"  # DG32 ink-2: captions, labels
  accent: "#d4a36e"          # DG32 copper: the one accent
  line: "#3d453b"            # DG32 border: the hairline
  hardware: "#bf7f3b"        # DG32 hardware copper: hardware-path elements
  safe: "#2f9e8c"            # DG32 signal teal: the safe state (bridge off) and nothing else
  white-overlay: "rgba(238,234,226,0.08)"

typography:
  # — reading ramp (Georgia) —
  body:        { fontFamily: "Helvetica Neue, Arial, sans-serif", cqw: 1.0,  weight: 400, lineHeight: 1.6, color: "text-secondary" }
  body-sm:     { fontFamily: "Helvetica Neue, Arial, sans-serif", cqw: 0.85, weight: 400, lineHeight: 1.6 }
  subtitle:    { fontFamily: "Helvetica Neue, Arial, sans-serif", cqw: 1.3,  weight: 400, lineHeight: 1.5 }
  label:       { fontFamily: "Helvetica Neue, Arial, sans-serif", px: 14, weight: 500, tracking: "3px", upper: true, color: "accent" }
  attribution: { fontFamily: "Helvetica Neue, Arial, sans-serif", px: 15, weight: 400, tracking: "2px", upper: true, color: "accent" }
  micro:       { fontFamily: "Helvetica Neue, Arial, sans-serif", px: 12, weight: 400, tracking: "2px", upper: true, color: "accent" }
  # — display / hero ramp (Georgia 400, sentence case) —
  h3:          { fontFamily: "Georgia", cqw: 1.8, weight: 400, lineHeight: 1.1 }
  timeline-headline:{ fontFamily: "Georgia", cqw: 1.9, weight: 400, lineHeight: 1.1 }
  card-headline:{ fontFamily: "Georgia", cqw: 2.0, weight: 400, lineHeight: 1.15 }
  stat-figure: { fontFamily: "Georgia", cqw: 3.0, weight: 400, lineHeight: 1.0 }
  quote-mark:  { fontFamily: "Georgia", cqw: 9.0, weight: 400, lineHeight: 0.5, color: "line" }
  h2:          { fontFamily: "Georgia", cqw: 4.0, weight: 400, lineHeight: 1.1 }
  h1:          { fontFamily: "Georgia", cqw: 6.2, weight: 400, lineHeight: 1.06 }
  display:     { fontFamily: "Georgia", cqw: 8.0, weight: 400, lineHeight: 1.04 }
...
The voice is a literary pairing: **Georgia** at weight 400 (the thin-stroke didone, never
bold, always sentence case) carries every headline, numeral, and quote mark in ink; **Georgia**
...
- **Georgia 400** (ink, sentence case) for display; **Georgia** body (gray) + labels (taupe, tracked).
...
Two ramps. The **reading ramp** (Georgia body 1.0cqw gray, labels in px taupe) carries copy + chrome;
...
- **Playfair at 400, ink, sentence case** — never bold, never uppercase, never taupe. **Georgia labels uppercase, 2–3px tracked, taupe.** Italic via Playfair italic for emphasis only.

## Depth & Surface

The flat plane is the only technique. Hierarchy from:

- **Type contrast** — Playfair serif vs Georgia sans; the 8cqw→0.7cqw scale.
...
**Ground** `{colors.bg-primary}`, `pad-x`. **Composes** geo-ring (right, ~34cqw, solid+dashed), vertical-line (left), label, display/h1. **Focal** a 2–3 line Playfair `display`/`h1` headline in ink (italic on the key word), left-anchored, with a taupe `label` above and an Georgia subtitle below. **Chrome** optional bottom meta row (Playfair value + taupe label). **Accent** the geo ring; optionally one `horizontal-accent` ink line (if it clears the meta). **Silence** ~55% empty. **Fixed** Playfair 400 ink sentence-case, ≤2 geo elements, 1px lines. **Free** title, ring placement, meta. **Density** sparse.
...
- **Georgia + Georgia via Google Fonts.** CJK pairing (Noto Serif SC 700/400) carries over; Playfair has no Hanzi italic — substitute weight/taupe for emphasis.
